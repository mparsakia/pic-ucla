   ...
   {  
      ...
      {  
         ...
         case WM_MOUSEMOVE:
            mouse_x = LOWORD(msg.lParam);
            mouse_y = HIWORD(msg.lParam);
            sprintf_s(buf, "(%f,%f)",disp_to_user_x(mouse_x),disp_to_user_y(mouse_y)); 
...
void GraphicWindow::color(int r, int g, int b)
{

	SelectObject(_hdc, GetStockObject(DC_PEN));
	SetDCPenColor(_hdc, RGB(r, g, b));
